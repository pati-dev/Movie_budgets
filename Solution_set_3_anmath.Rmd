...
I have fitted a locally libear loess model on the given dataset to predict the log value of movie budgets (to the base 10) from their year conditioned on their length. The thresholds for conditioning on length have been set at 90 minutes and 120 minutes, thereby categorizing movies into *short* (<90 minutes), *medium* (90-120 minutes), and *long* (>120 minutes). Below I have mentioned the reasoning behind my modeling choices in choronological order of the steps I followed to fit the model:

• The bivarate plots showcased that budget$1$ seemed to follow a linear trend with length and had high correlation (0.634). Furthermore, low correlation value (-0.08) for length vs. year was a strong hint that there might no interaction between these variables.

• The bivariate plot between budget and year was *almost* linear until 1994, post which it seemed to flatten out a bit. This coincides with the first professional use of camcorder for making movies, which was way cheaper than using the video cameras.
...
• Finally, I fitted an ordinary least squares linear model and a locally linear loess model with year on the x-axis and budget on the y-axis conditioned on length (to account for the interactions seen previously) and looked at their residual plots. While both the residual plots demonstrated homoskedasticity, the residuals for the loess model were much lesser than those for the OLS model. Hence, I went ahead with the loess fit for predicting movies budgets from year and length.
...
```
The 3D wireframe plot above confirms our observation that the budget for *long* movies has gone up at a much higher rate than that of the *medium* budget movies over the years, whereas the budget for *short* movies has remained pretty much constant. However, there is a clear dip in the movie budgets for *short* movies at around 1994 marking the use of handycams for making low-budget movies.
